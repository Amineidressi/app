...
db = Client("usess.hex")
bot = telebot.TeleBot("7080450678:AAEq4IkvLfd11sip-F5_8MLyhGzw4VtcIH4", num_threads=100, skip_pending=True, parse_mode="html")
rdod = ["بوت", "دادي", "بووت", "بوتي", "البوت"]
sudo   = [6799512209]#ADMIN
...
تح
    ...
    if t.startswith("استثمار "):
        ...
        if db.get(f"invest_{fid}"):
            ...
            time = datetime.timedelta(seconds=seconds)
            ftime = (datetime.datetime.min + time).time().strftime("%MM:%SS")
